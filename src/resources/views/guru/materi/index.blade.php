...
<head>
    ...
    <title>Kelola Materi - Pintar.id</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Outfit', sans-serif; }
        ...
        .btn-primary { background: var(--muted-sage); color: white; padding: 12px 24px; border-radius: 99px; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; transition: all 0.3s ease; border: none; cursor: pointer; font-size: 15px; }
        .btn-primary:hover { background: #7b846e; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(142, 150, 128, 0.3); }
        ...
        .badge-sd  { background: rgba(112, 161, 255, 0.2); color: #5B8BEB; }
        .badge-smp { background: rgba(217, 179, 130, 0.2); color: #B38F60; }
        .badge-sma { background: rgba(163, 124, 118, 0.2); color: #8a655f; }
    ...
    <main class="main-wrapper">
        ...
        <div class="content-body">
            ...
            <div class="section-header">
                <h2 class="section-title">Kelola Materi Belajar</h2>
                <a href="{{ route('guru.materi.create') }}" class="btn-primary">
                ...
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><strong>{{ $m->judul }}</strong>
                                    @if($m->deskripsi)
                                        <br><small style="color:rgba(61,43,31,0.5);font-weight:400;">{{ Str::limit($m->deskripsi, 50) }}</small>
                                    @endif
                                </td>
                                <td><span class="badge badge-{{ strtolower($m->jenjang) }}">{{ $m->jenjang }}</span></td>
                                <td>{{ $m->mapel ?? '-' }}</td>
                                <td>{{ $m->kelas ?? '-' }}</td>
                                <td>
                                    <div style="display:flex;gap:6px;flex-wrap:wrap;">
                                        @if($m->file_materi)
                                            <a href="{{ asset('storage/' . $m->file_materi) }}" target="_blank" class="btn-sm btn-file">📎 File</a>
                                        @endif
                                        @if($m->link_video)
                                            <a href="{{ $m->link_video }}" target="_blank" class="btn-sm btn-file">▶ Video</a>
                                        @endif
                                        @if(!$m->file_materi && !$m->link_video)
                                ...
                                <td>
                                    <div class="action-btns">
                                        <a href="{{ route('guru.materi.edit', $m->id_materi) }}" class="btn-sm btn-edit">✏️ Edit</a>
                                        <form action="{{ route('guru.materi.destroy', $m->id_materi) }}" method="POST" onsubmit="return confirm('Hapus materi ini?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="empty-state">
                                        ...
                                        Belum ada materi. Klik <strong>"Tambah Materi"</strong> untuk memulai.
                                    </div>
                                </td>
                            </tr>
                            @endforelse
