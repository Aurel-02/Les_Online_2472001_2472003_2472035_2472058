...
<head>
    ...
    <title>Jadwal Mengajar - Pintar.id</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Outfit',sans-serif; }
        ...
        .btn-primary { background:var(--warm-amber); color:var(--dark-oak); padding:12px 24px; border-radius:99px; font-weight:700; text-decoration:none; display:inline-flex; align-items:center; gap:8px; transition:all 0.3s ease; border:none; cursor:pointer; font-size:15px; }
        .btn-primary:hover { background:#c9a36a; transform:translateY(-2px); box-shadow:0 4px 12px rgba(217,179,130,0.4); }

        /* Jadwal Grid per Hari */
        .hari-section { margin-bottom:32px; }
        .hari-label {
        ...
        .hari-label.today { background:var(--muted-sage); color:white; border-color:var(--muted-sage); }
        .hari-label.today .hari-dot { background:rgba(255,255,255,0.6); }
        .hari-dot { width:8px; height:8px; border-radius:50%; background:var(--muted-sage); }
        ...
        .schedule-card:hover { transform:translateY(-4px); box-shadow:0 12px 32px rgba(61,43,31,0.08); background:white; }
        ...
        .schedule-mapel { font-size:18px; font-weight:800; color:var(--dark-oak); }
        .schedule-kelas { font-size:14px; font-weight:600; color:rgba(61,43,31,0.6); }
        .schedule-card-actions { display:flex; gap:8px; padding-top:8px; border-top:1px solid rgba(61,43,31,0.06); }
        .btn-sm { padding:7px 16px; border-radius:8px; font-size:13px; font-weight:600; text-decoration:none; display:inline-flex; align-items:center; gap:4px; }
        ...
        .empty-state { text-align:center; padding:64px 0; color:rgba(61,43,31,0.4); font-weight:500; font-size:16px; }
    ...
    <main class="main-wrapper">
        ...
        <div class="content-body">
            ...
            <div class="section-header">
                ...
                <a href="{{ route('guru.jadwal.create') }}" class="btn-primary">
            ...
            @if($jadwalList->isEmpty())
                <div class="empty-state">
                    ...
                    Belum ada jadwal. Klik <strong>"Tambah Jadwal"</strong> untuk mulai mengatur jadwal mengajar.
                </div>
            @else
                @php $hariIni = now()->locale('id')->dayName; @endphp
                @foreach($hariOrder as $hari)
                    @php $jadwalHari = $jadwalList->where('hari', $hari); @endphp
                    @if($jadwalHari->isNotEmpty())
                    <div class="hari-section">
                        <div class="hari-label {{ $hari == $hariIni ? 'today' : '' }}">
                            <span class="hari-dot"></span>
                            {{ $hari }}
                            @if($hari == $hariIni) &nbsp;— Hari Ini @endif
                        </div>
                            ...
                            <div class="schedule-card">
                                <div class="schedule-card-top">
                                    <div>
                                        ...
                                        <div class="schedule-kelas">{{ $j->kelas }}</div>
                                    </div>
                                ...
                                <div class="schedule-card-actions">
                                    <a href="{{ route('guru.jadwal.edit', $j->id_jadwal) }}" class="btn-sm btn-edit">✏️ Edit</a>
                                    <form action="{{ route('guru.jadwal.destroy', $j->id_jadwal) }}" method="POST" onsubmit="return confirm('Hapus jadwal ini?');">
                                        @csrf
                                        @method('DELETE')
