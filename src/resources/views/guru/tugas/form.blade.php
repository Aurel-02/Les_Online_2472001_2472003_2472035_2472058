...
<head>
    ...
    <title>{{ isset($tugas) ? 'Edit' : 'Buat' }} Tugas - Pintar.id</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Outfit',sans-serif; }
        ...
        .form-control:focus { border-color:var(--dusty-mauve); box-shadow:0 0 0 3px rgba(163,124,118,0.2); }
        ...
        .btn-submit { background:var(--dusty-mauve); color:white; padding:14px 28px; border-radius:12px; font-weight:600; font-size:16px; border:none; cursor:pointer; transition:all 0.3s ease; width:100%; margin-top:10px; }
        .btn-submit:hover { background:#8a655f; transform:translateY(-2px); box-shadow:0 4px 14px rgba(163,124,118,0.3); }
...
<body>
    ...
    <main class="main-wrapper">
        ...
        <div class="content-body">
            ...
            <h2 class="section-title">{{ isset($tugas) ? 'Edit Tugas' : 'Buat Tugas Baru' }}</h2>

            <div class="glass-card">
                <form action="{{ isset($tugas) ? route('guru.tugas.update', $tugas->id_tugas) : route('guru.tugas.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @if(isset($tugas)) @method('PUT') @endif

                    <div class="form-group">
                        <label class="form-label" for="judul">Judul Tugas <span>*</span></label>
                        <input type="text" id="judul" name="judul" class="form-control" value="{{ old('judul', $tugas->judul ?? '') }}" required placeholder="Contoh: Latihan Soal Persamaan Linear">
                    ...
                    <div class="form-row">
                        <div>
                            ...
                            <input type="text" id="mapel" name="mapel" class="form-control" value="{{ old('mapel', $tugas->mapel ?? '') }}" required placeholder="Contoh: Matematika">
                            @error('mapel') <div class="error-message">{{ $message }}</div> @enderror
                        </div>
                        <div>
                            <label class="form-label" for="kelas">Kelas <span>*</span></label>
                            <input type="text" id="kelas" name="kelas" class="form-control" value="{{ old('kelas', $tugas->kelas ?? '') }}" required placeholder="Contoh: X IPA 1">
                            @error('kelas') <div class="error-message">{{ $message }}</div> @enderror
                        </div>
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="deadline">Deadline <span>*</span></label>
                        <input type="datetime-local" id="deadline" name="deadline" class="form-control"
                            value="{{ old('deadline', isset($tugas) ? $tugas->deadline->format('Y-m-d\TH:i') : '') }}" required>
                        @error('deadline') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="deskripsi">Deskripsi / Instruksi</label>
                        <textarea id="deskripsi" name="deskripsi" class="form-control" placeholder="Tuliskan petunjuk pengerjaan tugas...">{{ old('deskripsi', $tugas->deskripsi ?? '') }}</textarea>
                        @error('deskripsi') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="file_tugas">Upload File Tugas (Opsional)</label>
                        <input type="file" id="file_tugas" name="file_tugas" class="form-control" accept=".pdf,.doc,.docx,.ppt,.pptx,.zip">
                        <div class="file-hint">Format: PDF, DOC, DOCX, PPT, PPTX, ZIP. Maks 20MB.</div>
                        @if(isset($tugas) && $tugas->file_tugas)
                            <a href="{{ asset('storage/' . $tugas->file_tugas) }}" target="_blank" class="current-file">📎 File saat ini (klik untuk lihat)</a>
                        @endif
                        @error('file_tugas') <div class="error-message">{{ $message }}</div> @enderror
                    </div>

                    <button type="submit" class="btn-submit">{{ isset($tugas) ? '💾 Simpan Perubahan' : '📝 Buat Tugas' }}</button>
                </form>
            </div>
        </div>
    </main>
