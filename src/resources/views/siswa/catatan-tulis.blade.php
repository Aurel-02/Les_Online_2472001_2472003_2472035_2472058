...
<nav>
    ...
    <div class="nav-right">
        <button type="submit" form="formCatatan" class="btn-save">Simpan Catatan</button>
...
<div class="page-body">
    ...
    <div class="left-split glass-card">
        <form id="formCatatan" action="{{ route('siswa.catatan.store') }}" method="POST" style="display:flex; flex-direction:column; height:100%;">
            @csrf
            <input type="hidden" name="mapel" value="{{ $mapel ?? 'Umum' }}">
            <input type="hidden" name="id_catatan" value="{{ isset($catatan) ? $catatan->id_catatan : '' }}">
            <div class="notes-header">
                <input type="text" name="judul" placeholder="Judul Catatan..." value="{{ isset($catatan) ? $catatan->judul : 'Catatan: ' . ($mapel ?? 'Baru') }}" required>
                <div class="notes-meta">Terakhir diedit: {{ isset($catatan) ? $catatan->updated_at->diffForHumans() : 'Baru saja' }}</div>
            </div>
            ...
            <div class="notes-body">
                <textarea name="isi_catatan" placeholder="Mulai mengetik catatanmu di sini...

Tips: Catat poin-poin penting, rumus, atau bagian yang belum kamu mengerti dari video di sebelah kanan." required>{{ isset($catatan) ? $catatan->isi_catatan : '' }}</textarea>
            </div>
        </form>
    </div>
